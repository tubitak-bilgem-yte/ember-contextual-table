...
test('it renders an input for filtering in simple form', function(assert) {
  ...
  this.render(hbs`{{dt-filterable-column-header filtervalueupdated=(action 'myAction') filterValue=filterValue placeholder='Filter...'}}`);
...
test('it yields header true and corresponding action in block form', function(assert) {
  ...
  addEventListener.bind(this)(assert);
  this.render(hbs`
    {{#dt-filterable-column-header filtervalueupdated=(action 'myAction') as |fc|}}
      {{fc.header}}-{{fc.footer}}
